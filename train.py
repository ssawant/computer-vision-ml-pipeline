...
from utils.general import (
    set_training_dir, Averager, save_model, save_train_loss_plot,
    show_tranformed_image, save_mAP, save_model_state
)
from utils.logging import (
    log, set_log, coco_log,
    set_summary_writer, tensorboard_loss_log,
    tensorboard_map_log, csv_log
)
...
def main(args):
        ...
        with torch.no_grad():
            for i in range(len(model.backbone)):
                if hasattr(model.backbone[i], 'weight'):
                    model.backbone[i].weight.copy_(
                        checkpoint['model_state_dict'][keys[2*counter]])
                    model.backbone[i].bias.copy_(
                        checkpoint['model_state_dict'][keys[2*counter+1]])
                    counter += 1
    ...
    for epoch in range(start_epochs, NUM_EPOCHS):
        ...
        tensorboard_loss_log('Train loss', np.array(
            train_loss_list_epoch), writer)
